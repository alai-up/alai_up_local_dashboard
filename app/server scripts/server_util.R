...
load_and_process_data <- function(input_df) {
  ...
  df <- input_df |>
    ...
    mutate(birth_date = case_when(str_length(birth_date) == 4 ~ as.Date(paste0(birth_date,"-01-01")),
                                  .default = as.Date(birth_date))) |>
...
get_IC_df <-function(input_df){
  ...
  IC_df <- basic_df |>
    ...
    mutate(
      PWH = 1,
      PWH1 = 1,
      Interested = data.table::fifelse(Interested == 1 | Prescribed == 1, 1, 0),
      Eligible = data.table::fifelse(Eligible == 1 | Prescribed == 1, 1, 0),
